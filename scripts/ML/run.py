...
All three steps write their output under the same experiment directory:
  data/experiment/<YYYY-MM-DD_HH-MM-SS>/
    presql.jsonl
    finsql.jsonl
    metrics.md
    raw_metrics.txt
...
Usage:
    uv run python -m scripts.ML.run \
...
EXP_DIR     = f"{ROOT_PATH}/data/experiment"
...
def main():
    ...
    # ══ Step 1: preSQL ═══════════════════════════════════════════════════════
    print(f"{sep}")
    print(f"  Step 1/3 — preSQL  [{presql_model}]")
    print(f"{sep}")
    ...
    from scripts.ML.gen_metrics import (
        _build_kmaps, _evaluate_file, _parse_spider_metrics,
        _comparison_table, _markdown_header,
    )
